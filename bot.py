# ==============================================================================
#                 Direct Link Generator - Telegram Bot
#
# Description:
#   A self-contained Telegram bot that generates direct download links from
...
def mediafire(url, session=None):
    ...
    try:
        ...
        try:
            ...
            raise DirectDownloadLinkException(f"ERROR: Could not fetch the URL. Details: {e}")
        ...
        if html.xpath("//div[ @class='passwordPrompt']"):
            ...
            try:
                text = session.post(url, data={"downloadp": _password}).text
                html = HTML(text)
            except Exception as e:
                raise DirectDownloadLinkException(f"ERROR: {e.__class__.__name__}") from e
            if html.xpath("//div[ @class='passwordPrompt']"):
        ...
        if final_link := findall(r"https?:\/\/download\d+\.mediafire\.com\/\S+\/\S+\/\S+", text):
            return final_link[0]

        # New logic from gemini.py, adapted for lxml
        download_button_list = html.xpath('//a[@id="downloadButton"]')
        final_link = None

        if download_button_list:
            download_button = download_button_list[0]
            if 'data-scrambled-url' in download_button.attrib:
                scrambled_url = download_button.attrib['data-scrambled-url']
                try:
                    decoded_bytes = b64decode(scrambled_url)
                    final_link = decoded_bytes.decode('utf-8')
                except Exception as e:
                    raise DirectDownloadLinkException(f"❌ Error: Failed to decode the scrambled URL. It might not be valid Base64. Details: {e}")
            elif 'href' in download_button.attrib and download_button.attrib['href'].startswith('http'):
                final_link = download_button.attrib['href']

        if not final_link:
            # Fallback to old logic if new logic fails
            if old_link := html.xpath('//a[@aria-label="Download file"]/@href'):
                 final_link = old_link[0]
            elif repair_link := html.xpath("//a[@class='retry']/@href"):
                return _repair_download(repair_link[0], session)
            else:
                raise DirectDownloadLinkException(
                    "❌ Error: Could not find a download link. The website's layout may have changed."
                )
        
        if "javascript:void(0)" in final_link:
            raise DirectDownloadLinkException("ERROR: Mediafire link is javascript:void(0), could not extract direct link.")

        if final_link.startswith("//"):
            final_url = f"https://{final_link[2:]}"
            if _password:
        ...
        if is_session_local and session:
            session.close()
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_html(
        "👋 Hello! I am a <b>Direct Link Generator Bot</b>.\n\n"
        "Send me a supported link and I will try to generate a direct download link for you.\n\n"
        "Use /help to see a list of supported sites."
    )
...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    reply = await message.reply_text("⏳ Generating link, please wait...")

    try:
        result = direct_link_generator(link)

        if isinstance(result, str):
            response_text = f"✅ **Direct Link Generated:**\n\n`{result}`"
        elif isinstance(result, tuple):
            response_text = f"✅ **Direct Link Generated:**\n\n`{result[0]}`"
            if len(result) > 1 and result[1]:
                 response_text += f"\n\n**Headers:**\n`{result[1]}`"
        elif isinstance(result, dict) and 'contents' in result:
            response_text = f"✅ **Folder Found: {result.get('title', 'N/A')}**\n\n"
            for i, item in enumerate(result['contents']):
                if i >= 20:
                    response_text += f"\n...and {len(result['contents']) - i} more files."
                    break
                response_text += f"📄 **{item['filename']}**\n`{item['url']}`\n\n"
        else:
            response_text = f"Could not process the result: {result}"

        await reply.edit_text(response_text, parse_mode=constants.ParseMode.MARKDOWN)

    except DirectDownloadLinkException as e:
        await reply.edit_text(f"❌ **Error:**\n{e}")
    except Exception as e:
        logger.error(f"An unexpected error occurred for link {link}: {e}", exc_info=True)
        await reply.edit_text("❌ An unexpected error occurred. Please try again later.")
...
def main() -> None:
    ...
    try:
        # This will now read from the .env file or the OS environment
        token = os.environ["TELEGRAM_BOT_TOKEN"]
    except KeyError:
        logger.error("FATAL: TELEGRAM_BOT_TOKEN not found in environment or .env file!")
        return
